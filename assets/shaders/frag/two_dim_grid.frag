...
Voxel traceByIndex(vec3 rayDir, vec3 rayOrgin, vec3 startingCoord, vec2 raybounds)
{
    ...
    float dstToTravelSq = raybounds.y * raybounds.y;

    vec3 lastPoint = max(ceil(abs(rayDir * (raybounds.y - raybounds.x))),1);

    float maxSteps = (lastPoint.x) + (lastPoint.y) + (lastPoint.z);
    for (float dst = 0; dst < maxSteps+1; ++dst)
    {
        ...
        dist.x = rayDir.x == 0 ? maxDst : abs((index.x - lastPointRel.x) / rayDir.x);
        dist.y = rayDir.y == 0 ? maxDst : abs((index.y - lastPointRel.y) / rayDir.y);
        dist.z = rayDir.z == 0 ? maxDst : abs((index.z - lastPointRel.z) / rayDir.z);
        
        vec3 nextIndex = vec3(0,0,0);
        if ( dist.x < dist.y && dist.x < dist.z)
        {
            nextIndex = vec3(directions.x,0,0);
            minDst = dist.x;
        }
        else if ( dist.y < dist.x && dist.y < dist.z)
        {
            nextIndex = vec3(0, directions.y, 0);
            minDst = dist.y;
        }
        else if ( dist.z < dist.x && dist.z < dist.y)
        {
            nextIndex = vec3(0, 0, directions.z);
            minDst = dist.z;
        }
        
        
        pointAlongRay = (lastPointRel + currentCoords) + rayDir * (minDst);
        currentCoords = currentCoords + (nextIndex);
        

        vec3 vec = pointAlongRay - rayOrgin;

        float dstSrqt = (vec.x * vec.x) + (vec.y * vec.y) + (vec.z * vec.z);
        if (dstSrqt >= dstToTravelSq 
            ||  currentCoords.x < 0 || currentCoords.x >= u_dimensions.x ||
            currentCoords.y < 0 || currentCoords.y >= u_dimensions.y ||
            currentCoords.z < 0 || currentCoords.z >= u_dimensions.z )
        {
        ...
        {
            currentVoxel = getVoxel(currentCoords, pointAlongRay, rayDir);
            if (currentVoxel.color.a > 0.01)
            {
                return currentVoxel;
            }
        }

        lastPointRel = pointAlongRay - currentCoords;
    }
...
void main()
{
    ...
    vec4 viewSpacePoint = u_projectionMatrix * clipSpacePoint;//inverse(u_projectionMatrix) * clipSpacePoint;
    ...
    if (raybounds.x < raybounds.y)
    {
        ...
        {
            // vec3 finalColor = vec3(0.1, 0.1, 0.1); // Ambient light
            // //for (int i = 0; i < 4; i++)
            // //{
            //     //if (lightDataColor[i].a != 0)
            //     //{
            //         float lenghtOfRay = 100;;
            //         vec4 lightPosition = vec4(5,100,0,0);//lightDataPos[i];
            //         vec3 L, R; // Light direction and reflected light direction.
            //         //if (lightPosition.w == 0.0) { // directional light
            //             L = normalize(lightPosition.xyz);
            ...
            FragColor = currentVoxel.color;// vec4(finalColor, currentVoxel.color.a);
        }
    }
